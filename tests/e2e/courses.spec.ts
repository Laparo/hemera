...
test.describe('Course List Page', () => {
  ...
  test('should display course cards or fallback', async ({ page }) => {
    ...
    if (hasCards) {
      ...
      const cardTitle = firstCard.locator('h2, h3, [data-testid="course-title"]');
      await expect(cardTitle.first()).toBeVisible();
      
      // Card should have description
      const cardDescription = firstCard.locator('[data-testid="course-description"]');
  ...
  test('should have structured data for courses', async ({ page }) => {
    ...
    for (const script of scripts) {
      ...
      if (data['@type'] === 'Course' || (Array.isArray(data) && data.some(item => item['@type'] === 'Course'))) {
